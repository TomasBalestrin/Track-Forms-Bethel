...
export async function POST(_request: Request, { params }: RouteContext) {
  try {
    ...
    const rules = await ruleService.getRules(supabase, pixel.id);
    ...
    for (const lead of newlyQualified) {
      ...
      if (res.success) {
        await leadService
          ...
          .catch((err) =>
            console.error(
              `[POST /api/pixels/${params.id}/leads/requalify] fb_sent_at`,
              err
            )
          );
        sent += 1;
        ...
        console.error(
          `[POST /api/pixels/${params.id}/leads/requalify] CAPI`,
          res.error
        );
      }
    }
    ...
    if (error instanceof LeadServiceError || error instanceof RuleServiceError) {
      console.error(
        `[POST /api/pixels/${params.id}/leads/requalify]`,
        error
      );
    } else {
      console.error(
